...
def main():
    ...
    elif args.task == "summarization_reference":
        log_dir = os.path.join(args.output_path, "summarization_reference", args.dataset_name, args.model_name, dataset_name, str(args.summary_length), time.strftime("%Y%m%d_%H%M%S"))
    ...
    if args.task == "bias_prediction":
        ...
        with torch.no_grad():
            for i, data in enumerate(tqdm(dataset, desc="Inferencing dataset")):
                ...
                target_class_list = ["left", "center", "right"]
                
                for reference_class in target_class_list:
                    try:
                        summary, data = model.process_question_summarization(data, reference_class, args.summary_length)                        
